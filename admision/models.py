...
class Expediente(models.Model):
    ...
    res_rect_ingreso = models.CharField(max_length=100, null=True, blank=True, default='')
    res_dirc_ingreso = models.CharField(max_length=100, null=True, blank=True, default='')
    ...
    def get_expediente_by_numero_documento(numero_documento):
        return Expediente.objects.filter(persona__numero_documento=numero_documento, is_active=True)
    
    @staticmethod
    def get_alumno_by_id_persona(id):
        return Expediente.objects.filter(persona__id=id, is_active=True).first()
