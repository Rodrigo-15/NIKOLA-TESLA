...
from academicos.models import (CursoGrupo, Cursos, Docente, Horario, Matricula,
                               Programa, RequisitoCurso)
from academicos.serializers import (CursoGrupoSerializer,
                                    CursoMatriculaSerializer,
                                    ExpedienteDocenteserializer,
                                    MatriculaSerializer, ProgramaSerializer)
...
def get_cursos_by_programa_id(request):
    if request.method == 'GET':
        ...
        obj_programa = Programa.objects.filter(id=programa_id).values(
            'codigo', 'cuotas', 'costo').first()
        obj_pago_ciclo = Matricula.objects.filter(expediente_id=expediente_id).distinct(
            'curso_grupo__curso__ciclo').values('curso_grupo__curso__ciclo').order_by('-curso_grupo__curso__ciclo').first()
        if obj_pago_ciclo == None:
            pago_ciclo = 0
        else:
            pago_ciclo = obj_pago_ciclo['curso_grupo__curso__ciclo']
        obj_pagos = Pago.objects.filter(
            expendiente_id=expediente_id, concepto__programa__codigo=obj_programa['codigo'])
        monto_total = 0
        ...
        if monto_total < (pagos_total-(4*pago_ciclo)):
            mensaje = "No puede realizar su matricula usted tiene una deuda de pension por un monto total de " + \
                str(pagos_total-monto_total) + \
                " Soles debe cancelar la deuda pendiente para poder continuar."
            return Response({"mensaje":  mensaje})
        # Tipo de programa
        obj_tipo_programa = Expediente.objects.filter(
            id=expediente_id).values('programa__tipo_id')
        tipo_programa = obj_tipo_programa[0]['programa__tipo_id']
        if tipo_programa == 1:
            matriculas = Matricula.objects.filter(Q(promedio_final__isnull=True) | Q(
                promedio_final__gte=14), expediente_id=expediente_id).exclude(is_retirado=True).exclude().values('curso_grupo__curso__id')
        elif tipo_programa == 2:
            matriculas = Matricula.objects.filter(Q(promedio_final__isnull=True) | Q(
                promedio_final__gte=15), expediente_id=expediente_id).exclude(is_retirado=True).exclude().values('curso_grupo__curso__id')

        # ciclos ya matriculados
        obj_ciclo = Matricula.objects.filter(expediente_id=expediente_id).distinct('curso_grupo__curso__ciclo').values(
            'curso_grupo__curso__ciclo').order_by('-curso_grupo__curso__ciclo').first()
        if obj_ciclo == None:
            ciclo = 1
        else:
            ciclo = obj_ciclo['curso_grupo__curso__ciclo']+1
        # cursos
        requisitos = RequisitoCurso.objects.filter().exclude(
            requisito_id__in=matriculas).values('curso_id')
        print(requisitos)
        cursos = Cursos.objects.filter(plan_estudio__programa_id=programa_id, ciclo__lte=ciclo).exclude(
            id__in=matriculas).exclude(id__in=requisitos).order_by('ciclo', 'id')
    ...
    if request.method == 'GET':
        ...
        for curso in cursos:
            curso_grupos = CursoGrupo.get_cursos_grupos_by_curso(
                curso, Periodo.get_periodo_activo().id).order_by('id')
            if len(curso_grupos) > 0:
                ...
                for grupo in curso_grupos:
                    docente = grupo.docente.full_name() if grupo.docente else "no asignado"
                    limite_registros = grupo.limite_registros
                    numero_registros = Matricula.get_numero_registros_by_curso_grupo_periodo(
...
def get_horarios_matriculados_by_expediente(request):
    if request.method == 'GET':
        ...
        for matricula in cursos_matriculados:
            ...
            grupos = []
            docente = curso_grupo.docente.full_name() if curso_grupo.docente else "no asignado"
            obj_grupo = {
                "id": curso_grupo.id,
...
def get_cursos_matriculados_by_expediente(request):
    if request.method == 'GET':
        ...
        for matricula in cursos_matriculados:
            ...
            grupo = curso_grupo.grupo
            data_final.append({**obj_curso, "grupo": grupo,
...
def get_alumnos_curso_grupo_by_id(request):
    if request.method == 'GET':
        ...
        for expediente in expedientes:
            ...
            serializer = ExpedienteAlumnoSerializer(alumno)
            alumnos.append(
                {**serializer.data, "promedio_final": promedio_final})
...
def get_estado_notas_curso_by_id(request):
    if request.method == 'GET':
        ...
        periodo_id = request.GET.get('periodo_id')
        matricula_obj = Matricula.objects.filter(
            periodo_id=periodo_id, curso_grupo_id=curso_grupo_id).first()
        if matricula_obj:
            return Response({"is_cerrado": matricula_obj.is_cerrado, "is_publicado": matricula_obj.is_publicado})
    ...
    if request.method == 'PUT':
        ...
        for alumno in alumnos:
            if alumno["promedio_final"] is not None:
                if float(alumno["promedio_final"]) >= 0.0 and float(alumno["promedio_final"]) <= 20.0:
                    matricula_obj = Matricula.objects.filter(
                        periodo_id=periodo_id, curso_grupo_id=curso_grupo_id, expediente_id=alumno["id"])
                    for matricula in matricula_obj:
                        matricula.promedio_final = float(
...
def get_persona(request):
    if request.method == 'GET':
        ...
        personas = Expediente.get_expediente_by_numero_documento(
            numero_documento)
        if not personas:
        ...
        for persona in personas:
            ...
            programas.append(
                {**obj_programa.data, "expediente_id": persona.id})
...
def get_matriculas(request):
    if request.method == 'GET':
        ...
        if programa == '0':
            print(programa)
            matriculas = Matricula.objects.all().filter(
                Q(expediente__persona__numero_documento__icontains=search) |
                Q(expediente__persona__apellido_paterno__icontains=search) |
                Q(expediente__persona__apellido_materno__icontains=search), periodo__id=periodo).distinct('expediente_id', 'periodo__id').order_by("-expediente_id")[:25]

        else:
            matriculas = Matricula.objects.all().filter(
                Q(expediente__persona__numero_documento__icontains=search) |
                Q(expediente__persona__apellido_paterno__icontains=search) |
                Q(expediente__persona__apellido_materno__icontains=search), periodo__id=periodo, expediente__programa__id=programa).distinct('expediente_id', 'periodo__id').order_by("-expediente_id")[:25]
        ...
        for expediente in expedientes:
            ...
            alumnos.append(
                {**serializer.data, "periodo": periodo, "fecha": fecha})
        return Response(alumnos)
...
def get_programas_by_periodo(request):
    if request.method == 'GET':
        periodo = request.GET.get('periodo_id')
        data = CursoGrupo.objects.all().filter(periodo__id=periodo).distinct(
            "curso__plan_estudio__programa__id")
        Programas = []
...
def get_alumno_by_dni(request):
    if request.method == 'GET':
        ...
        alumno = Expediente.objects.filter(
            persona__numero_documento=dni, is_active=True).first()
        if not alumno:
            return Response({"message": "Alumno no encontrado"}, status=404)
        serializer = ExpedienteAlumnoSerializer(alumno)
        return Response(serializer.data)
